"""
HYBRID BOND MODEL TEST
======================
Phase < pi: old formula exactly (don't fix what works)
Phase > pi: damped covalent + stronger ionic coefficient

The physics:
  phase < pi: standing wave peaks overlap between atoms = covalent resonance
  phase > pi: node appears between atoms, overlap weakens = ionic regime
  The ionic coefficient strengthens because there is less covalent screening
"""
import numpy as np
import math

d = 3
pi = np.pi
alpha_gwt = d**2 / (2**(d+1) * math.factorial(d+2)**(1.0/(d+1)) * pi**((d**2+d-1)/(d+1)))
F_mode = 2*d * pi**(2*d - 1)
m_e_gwt = F_mode * alpha_gwt**12 * 1.2209e22
E_H = alpha_gwt**2 * m_e_gwt * 1e6 / 2

f_pi = d**2 / (d**2 + 1)
alpha_n = 1 - f_pi / d
beta_n = (1 + f_pi) / 2
C_bond = pi / d
f_anti = 2*d / (2*d - 1)
c_ionic_cov = 1.0 / (2*d + 1)  # 1/7

Z_eff = {
    'H_1s': 1.0, 'Li_2s': 1.2792, 'B_2p': 2.4214,
    'C_2p': 3.1358, 'N_2p': 3.8340, 'O_2p': 4.4532,
    'F_2p': 5.0998, 'Na_3s': 2.5074, 'Cl_3p': 6.1161,
}

molecules = [
    ('H2',   1.401,  4.745, [('ss', 1)], 'H_1s',  'H_1s'),
    ('Li2',  5.051,  1.056, [('ss', 1)], 'Li_2s', 'Li_2s'),
    ('B2',   3.005,  3.02,  [('pi', 2)], 'B_2p',  'B_2p'),
    ('C2',   2.348,  6.32,  [('pi', 2), ('sp_sigma', 1), ('sp_sigma_anti', 1)], 'C_2p',  'C_2p'),
    ('N2',   2.074,  9.759, [('pp_sigma', 1), ('pi', 2)], 'N_2p',  'N_2p'),
    ('O2',   2.282,  5.213, [('pp_sigma', 1), ('pi', 2), ('pi_anti', 1)], 'O_2p',  'O_2p'),
    ('F2',   2.668,  1.660, [('pp_sigma', 1), ('pi', 2), ('pi_anti', 2)], 'F_2p',  'F_2p'),
    ('Na2',  5.818,  0.746, [('ss', 1)], 'Na_3s', 'Na_3s'),
    ('Cl2',  3.757,  2.514, [('pp_sigma', 1), ('pi', 2), ('pi_anti', 2)], 'Cl_3p', 'Cl_3p'),
    ('HF',   1.733,  5.869, [('sp', 1)], 'H_1s',  'F_2p'),
    ('CO',   2.132, 11.225, [('pp_sigma', 1), ('pi', 2)], 'C_2p',  'O_2p'),
    ('NO',   2.175,  6.497, [('pp_sigma', 1), ('pi', 2), ('pi_anti', 1)], 'N_2p',  'O_2p'),
    ('OH',   1.834,  4.392, [('sp', 1)], 'O_2p',  'H_1s'),
    ('HCl',  2.409,  4.434, [('sp', 1)], 'H_1s',  'Cl_3p'),
    ('LiH',  3.015,  2.515, [('ss', 1)], 'Li_2s', 'H_1s'),
    ('LiF',  2.955,  5.939, [('sp', 1)], 'Li_2s', 'F_2p'),
    ('BH',   2.329,  3.42,  [('sp', 1)], 'B_2p',  'H_1s'),
    ('CH',   2.116,  3.47,  [('sp', 1)], 'C_2p',  'H_1s'),
    ('NH',   1.958,  3.57,  [('sp', 1)], 'N_2p',  'H_1s'),
    ('BF',   2.386,  7.81,  [('pp_sigma', 1), ('pi', 2)], 'B_2p',  'F_2p'),
    ('CN',   2.214,  7.72,  [('pp_sigma', 1), ('pi', 2)], 'C_2p',  'N_2p'),
    ('NaH',  3.566,  1.97,  [('ss', 1)], 'Na_3s', 'H_1s'),
    ('NaCl', 4.461,  4.23,  [('sp', 1)], 'Na_3s', 'Cl_3p'),
    ('H2O',  1.809,  5.117, [('sp', 1)], 'O_2p',  'H_1s'),
]


def get_n(orb): return int(orb.split('_')[1][0])
def get_l(orb): return {'s': 0, 'p': 1}[orb.split('_')[1][1]]
def has_nodes(orb): return min(get_n(orb) - get_l(orb) - 1, 1)
def get_params(orb):
    n, l, h = get_n(orb), get_l(orb), has_nodes(orb)
    return n, 2 + (1-2*l)*alpha_n*h, 1 + (1-2*l)*beta_n*h


def compute_base(mol):
    """Common computation for both old and new."""
    name, R, De_exp, bonds, orb1, orb2 = mol
    n1, a1, b1 = get_params(orb1)
    n2, a2, b2 = get_params(orb2)
    E_scale = np.sqrt(E_H/n1**a1 * E_H/n2**a2)
    sigma_phase = R * (1.0/n1**b1 + 1.0/n2**b2)
    eps1 = E_H * (Z_eff[orb1]/n1)**2
    eps2 = E_H * (Z_eff[orb2]/n2)**2
    de = abs(eps1 - eps2)
    npb = sum(c for bt, c in bonds if 'pi' in bt and 'anti' not in bt)
    npa = sum(c for bt, c in bonds if 'pi' in bt and 'anti' in bt)
    ifa = (npa >= npb) if npb > 0 else True

    D_cov = 0
    for bt, cnt in bonds:
        ph = sigma_phase if ('sigma' in bt or bt in ('ss', 'sp')) else sigma_phase * f_pi
        cont = C_bond * E_scale * abs(np.sin(ph))
        if 'anti' in bt:
            fa = 1.0 if ('sigma' in bt or ifa) else f_anti
            D_cov -= cnt * fa * cont
        else:
            D_cov += cnt * cont

    return R, D_cov, sigma_phase, de, E_scale


def compute_old(mol):
    """Old formula: |sin| + c_ionic_cov everywhere."""
    R, D_cov, sigma_phase, de, E_scale = compute_base(mol)
    Vc = max(abs(D_cov), 0.01)
    q = de / np.sqrt(de**2 + (2*Vc)**2) if de > 0 else 0
    Di = c_ionic_cov * q**2 * 2 * E_H / R
    return D_cov + Di, sigma_phase, q


def compute_hybrid(mol, c_ion, damp_scale):
    """Hybrid: old for phase<pi, damped+stronger ionic for phase>pi."""
    R, D_cov, sigma_phase, de, E_scale = compute_base(mol)

    if sigma_phase < pi:
        # COV: exactly the old formula
        Vc = max(abs(D_cov), 0.01)
        q = de / np.sqrt(de**2 + (2*Vc)**2) if de > 0 else 0
        Di = c_ionic_cov * q**2 * 2 * E_H / R
        regime = 'COV'
    else:
        # ION: damp covalent, strengthen ionic
        damping = np.exp(-damp_scale * (sigma_phase - pi) / pi)
        D_cov = D_cov * damping
        Vc = max(abs(D_cov), 0.01)
        q = de / np.sqrt(de**2 + (2*Vc)**2) if de > 0 else 0
        Di = c_ion * q**2 * 2 * E_H / R
        regime = 'ION'

    return D_cov + Di, regime, sigma_phase, q


# ================================================================
# SCAN
# ================================================================
best = (100, 0, 0)
for c_ion in np.arange(0.1, 2.0, 0.01):
    for ds in np.arange(0.5, 5.0, 0.1):
        errs = []
        for mol in molecules:
            De_pred = compute_hybrid(mol, c_ion, ds)[0]
            errs.append(abs((De_pred - mol[2]) / mol[2] * 100))
        avg = np.mean(errs)
        if avg < best[0]:
            best = (avg, c_ion, ds)

avg, c_ion_best, ds_best = best

print("=" * 95)
print("  HYBRID: old formula (phase<pi) + damped cov + stronger ionic (phase>pi)")
print("=" * 95)
print(f"  c_ion = {c_ion_best:.3f}  damp_scale = {ds_best:.1f}")
print()

# Check d-fractions for c_ion
fracs = [("1/(2d+1)=1/7", 1/(2*d+1)), ("1/d=1/3", 1/d),
         ("(d-1)/d=2/3", (d-1)/d), ("d/(d+1)=3/4", d/(d+1)),
         ("1", 1.0), ("d/(d-1)=3/2", d/(d-1)), ("d=3", float(d))]
print("  c_ion nearest d-fractions:")
for label, val in sorted(fracs, key=lambda x: abs(x[1] - c_ion_best))[:3]:
    print(f"    {label:20s} = {val:.4f}  (diff={abs(val-c_ion_best):.4f})")

dfracs = [("1", 1.0), ("d-1=2", d-1), ("d=3", float(d)), ("d+1=4", d+1),
          ("pi", pi), ("d/2=1.5", d/2), ("(d+1)/d=4/3", (d+1)/d)]
print(f"  damp_scale nearest d-values:")
for label, val in sorted(dfracs, key=lambda x: abs(x[1] - ds_best))[:3]:
    print(f"    {label:20s} = {val:.4f}  (diff={abs(val-ds_best):.4f})")
print()

# Detailed comparison
hdr = (f"{'Mol':<6} {'De_obs':>6} {'old':>6} {'e_old':>7} "
       f"{'new':>6} {'e_new':>7} {'reg':>4} {'ph/pi':>6} {'q':>5}  verdict")
print(hdr)
print('-' * 82)

old_errs = []
new_errs = []
for mol in molecules:
    De_old, phase, q_old = compute_old(mol)
    De_new, regime, _, q_new = compute_hybrid(mol, c_ion_best, ds_best)
    eo = (De_old - mol[2]) / mol[2] * 100
    en = (De_new - mol[2]) / mol[2] * 100
    old_errs.append(abs(eo))
    new_errs.append(abs(en))
    v = 'BETTER' if abs(en) < abs(eo) - 1 else ('WORSE' if abs(en) > abs(eo) + 1 else '~')
    print(f"{mol[0]:<6} {mol[2]:6.3f} {De_old:6.3f} {eo:+6.1f}% "
          f"{De_new:6.3f} {en:+6.1f}% {regime:>4} {phase/pi:6.3f} {q_new:5.3f}  {v}")

print()
wo5 = sum(1 for e in old_errs if e < 5)
wo10 = sum(1 for e in old_errs if e < 10)
wo20 = sum(1 for e in old_errs if e < 20)
wn5 = sum(1 for e in new_errs if e < 5)
wn10 = sum(1 for e in new_errs if e < 10)
wn20 = sum(1 for e in new_errs if e < 20)
print(f"OLD:  avg={np.mean(old_errs):.1f}%, med={np.median(old_errs):.1f}%, "
      f"<5%:{wo5}/24, <10%:{wo10}/24, <20%:{wo20}/24")
print(f"NEW:  avg={np.mean(new_errs):.1f}%, med={np.median(new_errs):.1f}%, "
      f"<5%:{wn5}/24, <10%:{wn10}/24, <20%:{wn20}/24")
nb = sum(1 for i in range(24) if new_errs[i] < old_errs[i] - 1)
nw = sum(1 for i in range(24) if new_errs[i] > old_errs[i] + 1)
print(f"Better: {nb}, Worse: {nw}, ~Same: {24 - nb - nw}")

# Test d-fraction candidates for c_ion
print()
print("Exact d-fraction tests for c_ion (damp_scale=best):")
for lbl, ci in [("1/7", 1/(2*d+1)), ("1/3", 1/d), ("2/3", (d-1)/d),
                ("3/4", d/(d+1)), ("1", 1.0), ("3/2", d/(d-1)),
                (f"best={c_ion_best:.2f}", c_ion_best)]:
    errs = [abs((compute_hybrid(m, ci, ds_best)[0] - m[2]) / m[2] * 100)
            for m in molecules]
    w5 = sum(1 for e in errs if e < 5)
    w10 = sum(1 for e in errs if e < 10)
    w20 = sum(1 for e in errs if e < 20)
    print(f"  c_ion={ci:.4f} ({lbl:10s}): avg={np.mean(errs):.1f}%, "
          f"med={np.median(errs):.1f}%, <5%={w5}, <10%={w10}, <20%={w20}")
